...
export default class SignaturePad {
  // Public stuff
  public screenDimensions : {width:number, height:number} = {width:0, height:0};
 ...
 // Stores all points in groups (one group per line or dot)
  ...
  constructor(
    private canvas: any,
    private options: Options = {screenDimensions:{width:300,height:300}},
  ) {
    this.screenDimensions = options.screenDimensions;

    this.velocityFilterWeight = options.velocityFilterWeight || 0.7;
    this.minWidth = options.minWidth || 8;
    this.maxWidth = options.maxWidth || 27;
  ...
  private _drawCurveSegment(x: number, y: number, width: number): void {
    ...
    ctx.moveTo(
      ...
      this._ctx.height*(y/this.screenDimensions.height)
    );
    ctx.arc(
      this._ctx.width*(x/this.screenDimensions.width),
      this._ctx.height*(y/this.screenDimensions.height),
